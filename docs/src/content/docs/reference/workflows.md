...
| `squad-cast-scan` | You want to auto-compose a squad for the selected project. | The roster's "Hire" action. | One agent turn calls `squad_propose_cast` to run a confined read-only repo scan and publish a proposal to approve or discard. | Authoring one member (`squad-genesis`) or approving a proposal (the Proposed squad board actions). |
