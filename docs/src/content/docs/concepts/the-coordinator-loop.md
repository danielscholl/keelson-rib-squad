...
| Pick a method | Same turn as assess | Names the next speaker and how they should work: a plain dispatch, a confined coding turn, or authoring a reusable workflow |
...
- **The goal is met** and, where applicable, the review and verification checks
  clear.
- **It runs out of rounds.** There is a hard ceiling on how many rounds a single run
  will take (24 by default), configurable when the run is started.
...
is acting and what they are doing. Once any work has been attributed, a "worked by"
line lists who did it and on which provider.
