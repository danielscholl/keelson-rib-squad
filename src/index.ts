...
- If there is no activeTheme, this squad has no cast yet and this member founds it: pick the ensemble that best fits this project, preferring one NOT in themeHistory (most-recent last) so a re-cast squad does not repeat the ensemble it just retired.
- Otherwise, prefer reusing the active ensemble (activeTheme) while it has remainingCapacity, picking a free character whose role fits (set castAs.themeId to activeTheme.id). If it has no room, or none of its characters fit this role, either pick a fitting catalog ensemble (castAs.themeId, one of catalog[].id — the catalog is a starting set, not a limit) or draw on an ensemble outside it (castAs.newThemeLabel — never both themeId and newThemeLabel). A newThemeLabel MUST name a REAL, widely-known movie/TV/book work (e.g. "The Expanse", "The Matrix", "Apollo 13"), and the characterName MUST be one of that work's ACTUAL characters. Do NOT make up a team name or invent characters: "The Flux Navigators" with an invented crew is WRONG; "The Expanse" with Holden or Naomi is right. If no real work fits the brief, reuse a catalog ensemble rather than fabricating one. Pick a characterName that is NOT in takenCharacterNames. The listed characterNames are EXAMPLES of who each work fields, not the set you may pick from: name whichever of that work's real characters best fits this role and project, listed or not. Write a short personality and backstory for it in your own words, written for THIS project — they are what the member is cast with. If a pin is present, only cast within that exact ensemble (name it via newThemeLabel matching the pin if it isn't a known ensemble yet, using that work's real characters).
